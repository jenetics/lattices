...
class UnknownDoubleQuantileEstimator extends DoubleQuantileEstimator {
     ...
     * @param precomputeEpsilon the epsilon for which quantiles shall be
     * precomputed; set this value <=0.0 if nothing shall be precomputed.
    ...
    protected static void sortAscendingByLevel(DoubleBuffer[] fullBuffers) {
        new ObjectArrayList<>(fullBuffers).quickSortFromTo(0, fullBuffers.length - 1, new java.util.Comparator<Object>() {
