...
public abstract class AbstractBooleanList extends AbstractList<Boolean> {
	 ...
	 * are undefined: in particular, the call may enter an infinite loop.  If
	 * the receiver contains multiple elements equal to the specified object,
	 ...
	 * method may decide <b>not to copy the array</b>.
	...
	public boolean forEach(BooleanProcedure procedure) {
		for (int i = 0; i < size; )
			if (!procedure.apply(get(i++))) return false;
		return true;
	}
	 ...
	 * succeeding
	 * elements to the left (reduces their index). This call booleanens the list
	...
	public void replaceFromWith(int from, java.util.Collection<? extends Boolean> other) {
		...
		java.util.Iterator<? extends Boolean> e = other.iterator();
		int index = from;
