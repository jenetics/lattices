...
public class Sorting {
	...
	*/
     ...
     */
    public static int binarySearchFromTo(Object[] list, Object key, int from, int to, java.util.Comparator<? super Object> comparator) {
        Object midVal;
        while (from <= to) {
            int mid = (from + to) / 2;
            midVal = list[mid];
...
// end inline
     ...
     * <tt>c.compare(e1, e2)</tt> must not throw a
     * <tt>ClassCastException</tt> for any elements <tt>e1</tt> and
     * <tt>e2</tt> in the range).<p>
     * <p>
...
// Postprocessing phase: change 0.0's to -0.0's as required
     ...
     * @see Comparator
     */
    public static void quickSort(Object[] a, int fromIndex, int toIndex, Comparator<? super Object> c) {
        rangeCheck(a.length, fromIndex, toIndex);
        quickSort1(a, fromIndex, toIndex - fromIndex, c);
    }
     ...
     * @see Comparator
     */
    public static void quickSort(Object[] a, Comparator<? super Object> c) {
     ...
     * Sorts the specified sub-array of chars into ascending order.
     */
    private static void quickSort1(Object[] x, int off, int len, Comparator<? super Object> comp) {
        // Insertion sort on smallest arrays
        if (len < SMALL) {
            for (int i = off; i < len + off; i++)
                for (int j = i; j > off && comp.compare(x[j - 1], x[j]) > 0; j--)
