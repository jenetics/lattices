...
public class Partitioning {
     ...
     * Takes as first argument the index <tt>a</tt> within the generic splitters
     * <tt>s</tt>. Takes as second argument the index <tt>b</tt> within the
     ...
     * key, or <tt>list.length</tt>, if all elements in the list are less than
    ...
    /**
     * Same as
     * {@link #dualPartition(int[], int[], int, int, int[], int, int, int[])}
    ...
    /**
     * Same as {@link #dualPartition(int[], int[], int, int, int)} except that
     * it <i>synchronously</i> partitions <tt>double[]</tt> rather than
     * <tt>int[]</tt> arrays.
     */
     ...
     * except that this method <i>synchronously</i> partitions two arrays at the
     * same time; both arrays are partially sorted according to the elements of
     ...
     * secondary array is also moved from index A to B.
     * <p>
     * <b>Use cases:</b>
     * <p>
    ...
    /**
     * Same as {@link #partition(int[], int, int, int[], int, int, int[])}
     * except that it <i>generically</i> partitions arbitrary shaped data (for
     * example matrices or multiple arrays) rather than <tt>int[]</tt> arrays.
     * <p>
     ...
     * The comparison function determines whether <tt>s[a]</tt> is equal, less
     * or greater than <tt>g[b]</tt>. This method can then decide to swap the
     * data <tt>g[b]</tt> with the data <tt>g[c]</tt> (yes, <tt>c</tt>, not
     * <tt>a</tt>). It calls a user provided {@link cern.colt.Swapper} object
     * that knows how to swap the data of these two indexes.
     * <p>
     ...
     * considered. The method considers the elements <tt>g[from] .. g[to]</tt>.
     * @param splitFrom the index of the first splitter element to be
     * considered.
     * @param splitTo the index of the last splitter element to be considered.
     * The method considers the splitter elements <tt>s[splitFrom] ..
     * s[splitTo]</tt>.
     * @param splitIndexes a list into which this method fills the indexes of
     * elements delimiting intervals. Upon return
     * <tt>splitIndexes[splitFrom..splitTo]</tt> will be set accordingly.
     ...
     * generic data. Takes as first argument the index <tt>a</tt> within the
     * generic splitters <tt>s</tt>. Takes as second argument the index
     * <tt>b</tt> within the generic data <tt>g</tt>.
     * @param comp2 the comparator to determine the order of the generic data.
     * Takes as first argument the index <tt>a</tt> within the generic data
     * <tt>g</tt>. Takes as second argument the index <tt>b</tt> within the
     * generic data <tt>g</tt>.
     * @param comp3 the comparator comparing a splitter with another splitter.
     * Takes as first argument the index <tt>a</tt> within the generic splitters
     * <tt>s</tt>. Takes as second argument the index <tt>b</tt> within the
     * generic splitters <tt>g</tt>.
     * @param swapper an object that knows how to swap the elements at any two
     * indexes (a,b). Takes as first argument the index <tt>b</tt> within the
     * generic data <tt>g</tt>. Takes as second argument the index <tt>c</tt>
     * within the generic data <tt>g</tt>.
     *
     * <p>
     * Tip: Normally you will have <tt>splitIndexes.length == s.length</tt> as
     * well as <tt>from==0, to==g.length-1</tt> and <tt>splitFrom==0,
     * splitTo==s.length-1</tt>.
     ...
     */
    private static int med3(Object[] x, int a, int b, int c, java.util.Comparator<? super Object> comp) {
    ...
    /**
     * Same as {@link #partition(int[], int, int, int[], int, int, int[])}
     * except that it partitions <tt>double[]</tt> rather than <tt>int[]</tt>
     * arrays.
     */
     ...
     * <p>
     * The method modifies the list to be <tt>list = (4, 4, 3, 6, 7, 5, 6,
     * 50)</tt> and returns the <tt>splitIndexes = (2, 6, 6)</tt>. In other
     * words,
     * <ul>
     ...
     * considered. The method considers the elements <tt>list[from] ..
     * list[to]</tt>.
     * @param splitters the values at which the list shall be split into
     * intervals. Must be sorted ascending and must not contain multiple
     * identical values. These preconditions are not checked; be sure that they
     * are met.
     * @param splitFrom the index of the first splitter element to be
     * considered.
     * @param splitTo the index of the last splitter element to be considered.
     * The method considers the splitter elements <tt>splitters[splitFrom] ..
     * splitters[splitTo]</tt>.
     * @param splitIndexes a list into which this method fills the indexes of
     * elements delimiting intervals. Upon return
     * <tt>splitIndexes[splitFrom..splitTo]</tt> will be set accordingly.
     * Therefore, must satisfy <tt>splitIndexes.length > splitTo</tt>.
     * <p>
     * Tip: Normally you will have <tt>splitIndexes.length ==
     * splitters.length</tt> as well as <tt>from==0, to==list.length-1</tt> and
	...
	*/
     ...
     * <p>
     * The method modifies the list to be <tt>list = (4, 4, 3, 50, 6, 7, 5,
     * 6)</tt> and returns the split index <tt>2</tt>. In other words,
     * <ul>
     * <li>All values <tt>list[0..2]</tt> fall into <tt>[-infinity,5)</tt>.
	...
	*/
    ...
    /**
     * Same as {@link #partition(int[], int, int, int[], int, int, int[])}
     * except that it partitions <tt>Object[]</tt> rather than <tt>int[]</tt>
     * arrays.
     */
    public static void partition(Object[] list, int from, int to, Object[] splitters, int splitFrom, int splitTo, int[] splitIndexes, java.util.Comparator<? super Object> comp) {
     ...
     */
    public static int partition(Object[] list, int from, int to, Object splitter, java.util.Comparator<? super Object> comp) {
    ...
    /**
     * Equivalent to <tt>partition(list.elements(), from, to,
     * splitters.elements(), 0, splitters.size()-1,
    ...
    /**
     * Same as
     * {@link #triplePartition(int[], int[], int[], int, int, int[], int, int,
     * int[])} except that it <i>synchronously</i> partitions <tt>double[]</tt>
    ...
    /**
     * Same as {@link #partition(int[], int, int, int[], int, int, int[])}
     * except that this method <i>synchronously</i> partitions three arrays at
     * the same time; all three arrays are partially sorted according to the
     * elements of the primary array. In other words, each time an element in
     * the primary array is moved from index A to B, the correspoding element
     * within the secondary array as well as the corresponding element within
     * the tertiary array are also moved from index A to B.
     * <p>
     * <b>Use cases:</b>
     * <p>
