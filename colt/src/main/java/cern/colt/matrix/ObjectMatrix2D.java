 ...
 * an element at a coordinate <tt>column&lt;0 || column&gt;=columns() ||
 * row&lt;0 || row&gt;=rows()</tt> will throw an
...
public abstract class ObjectMatrix2D extends AbstractMatrix2D {
     ...
     * a(i-1), f(get(row,column)) )</tt> and terminators are <tt>a(1) ==
     * f(get(0,0)), a(0)==null</tt>.
     * <p>
     * <b>Example:</b>
     * <pre>
     ...
     * f(get(row,column),other.get(row,column)) )</tt> and terminators are
     ...
     * @return the aggregated measure.
     * @throws IllegalArgumentException if <tt>columns() != other.columns() ||
     * rows() != other.rows()</tt>
     * @see cern.jet.math.Functions
     */
     ...
     * @throws IllegalArgumentException if <tt>values.length != rows() || for
     * any 0 &lt;= row &lt; rows(): values[row].length != columns()</tt>.
     */
    ...
    public boolean equals(Object otherObj, boolean testForEquality) { //delta
        ...
        if (!testForEquality) {
            ...
            for (int row = rows; --row >= 0; ) {
                for (int column = columns; --column >= 0; ) {
                    if (!(getQuick(row, column) == null ? other.getQuick(row, column) == null : getQuick(row, column).equals(other.getQuick(row, column))))
                        return false;
                }
            }
        }
     ...
     * @throws IndexOutOfBoundsException if <tt>column&lt;0 ||
     * column&gt;=columns() || row&lt;0 || row&gt;=rows()</tt>
     */
     ...
     */
    public void getNonZeros(IntArrayList rowList, IntArrayList columnList, ObjectArrayList<Object> valueList) {
        rowList.clear();
        columnList.clear();
        valueList.clear();
        int r = rows;
     ...
     * Precondition (unchecked): <tt>0 &lt;= column &lt; columns() && 0 &lt;=
     * row &lt; rows()</tt>.
     *
     * @param row the index of the row-coordinate.
     * @param column the index of the column-coordinate.
     ...
     * this matrix, and vice-versa. Use idioms like <tt>result =
    ...
    /**
     * Constructs and returns a new <i>sub-range view</i> that is a <tt>height x
     * width</tt> sub matrix starting at <tt>[row,column]</tt>.
     * <p>
     ...
     * any attempt to access a cell at a coordinate <tt>column&lt;0 ||
     * column&gt;=view.columns() || row&lt;0 || row&gt;=view.rows()</tt> will
     * throw an <tt>IndexOutOfBoundsException</tt>.
     *
     * @param row The index of the row-coordinate.
     * @param column The index of the column-coordinate.
     ...
     * @return the new view.
     * @throws IndexOutOfBoundsException if <tt>column<0 || width<0 ||
     * column+width>columns() || row<0 || height<0 || row+height>rows()</tt>
     */
     ...
     * holding the indicated cells. There holds <tt>view.rows() ==
     * rowIndexes.length, view.columns() == columnIndexes.length</tt> and
     * <tt>view.get(i,j) == this.get(rowIndexes[i],columnIndexes[j])</tt>.
     ...
     * {@link cern.colt.matrix.objectalgo.Sorting#sort(ObjectMatrix2D, int)}.
     ...
     * <tt>this.get(i*rowStride,j*columnStride)</tt> for all <tt>i =
     * 0..rows()/rowStride - 1, j = 0..columns()/columnStride - 1</tt>. The
     * returned view is backed by this matrix, so changes in the returned view
     * are reflected in this matrix, and vice-versa.
     *
     ...
     * @return a new view.
     * @throws IndexOutOfBoundsException if <tt>rowStride<=0 ||
     * columnStride<=0</tt>.
     */
     ...
     * <tt>[rows()-1,columns()-1]</tt> to <tt>[0,0]</tt>, as demonstrated by
     * this snippet:
     * <pre>
     * for (int row=rows; --row >=0;) {
     *    for (int column=columns; --column >= 0;) {
