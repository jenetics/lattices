...
public class Partitioning {
    ...
    /**
     * Same as
     * {@link cern.colt.Partitioning#partition(int[], int, int, int[], int, int,
     * int[])} except that it <i>synchronously</i> partitions the rows of the
     * given matrix by the values of the given matrix column; This is
     * essentially the same as partitioning a list of composite objects by some
     * instance variable; In other words, two entire rows of the matrix are
     * swapped, whenever two column values indicate so.
     * <p>
     ...
     * @param rowIndexes the index of the i-th row; is modified by this method
     * to reflect partitioned indexes.
     ...
     * @param splitters the values at which the rows shall be split into
     ...
     * rows delimiting intervals. Upon return
     * <tt>splitIndexes[splitFrom..splitTo]</tt> will be set accordingly.
     * Therefore, must satisfy <tt>splitIndexes.length >=
     * splitters.length</tt>.
     */
     ...
     * are met.
     * @param splitIndexes a list into which this method fills the indexes of
     * rows delimiting intervals. Therefore, must satisfy
    ...
    /**
     * Same as {@link #partition(ObjectMatrix2D, int, Object[], int[])}
     ...
     * formally, there must hold: <br> There exists an <tt>i</tt> such that
	...
	*/
    ...
    /**
     * Same as {@link #partition(ObjectMatrix2D, int, Object[], int[])} except that it
