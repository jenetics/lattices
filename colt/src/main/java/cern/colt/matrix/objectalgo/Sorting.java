...
public class Sorting extends cern.colt.PersistentObject {
     ...
     */
    public ObjectMatrix1D sort(final ObjectMatrix1D vector, final java.util.Comparator<? super Object> c) {
        int[] indexes = new int[vector.size()]; // row indexes to reorder instead of matrix itself
        for (int i = indexes.length; --i >= 0; ) indexes[i] = i;
     ...
     * view <tt>matrix.viewRow(row).viewColumn(column)</tt>.
     * <b>Note that the original matrix is left unaffected.</b>
     * @throws IndexOutOfBoundsException if <tt>row < 0 || row >= matrix.rows()
     * || column < 0 || column >= matrix.columns()</tt>.
     */
    @SuppressWarnings({"rawtypes", "unchecked"})
