 ...
 * {@link cern.colt.map.OpenIntObjectHashMap}, which is a compact and performant
 * hashing technique.
 * <p>
...
public class SparseObjectMatrix3D extends ObjectMatrix3D {
     ...
     * and have exactly the same number of rows in in every slice and exactly
     * the same number of columns in in every row.
     * <p>
     * The values are copied. So subsequent changes in <tt>values</tt> are not
     * reflected in the matrix, and vice-versa.
     *
     * @param values The values to be filled into the new matrix.
     * @throws IllegalArgumentException if <tt>for any 1 &lt;= slice &lt;
     * values.length: values[slice].length != values[slice-1].length</tt>.
     * @throws IllegalArgumentException if <tt>for any 1 &lt;= row &lt;
     * values[0].length: values[slice][row].length !=
     ...
     * @throws IllegalArgumentException if <tt>(double)slices*columns*rows >
     * Integer.MAX_VALUE</tt>.
     * @throws IllegalArgumentException if <tt>slices<0 || rows<0 ||
     * columns<0</tt>.
     */
     ...
     * related to memory usage see {@link cern.colt.map.OpenIntObjectHashMap}.
     *
     * @param slices the number of slices the matrix shall have.
     * @param rows the number of rows the matrix shall have.
     * @param columns the number of columns the matrix shall have.
     * @param initialCapacity the initial capacity of the hash map. If not
     * known, set <tt>initialCapacity=0</tt> or small.
     ...
     * @throws IllegalArgumentException if <tt>(Object)slices*columns*rows >
     * Integer.MAX_VALUE</tt>.
     * @throws IllegalArgumentException if <tt>slices<0 || rows<0 ||
     * columns<0</tt>.
     */
    protected SparseObjectMatrix3D(int slices, int rows, int columns, AbstractIntObjectMap<Object> elements, int sliceZero, int rowZero, int columnZero, int sliceStride, int rowStride, int columnStride) {
