 ...
 * to access an element at a coordinate <tt>slice&lt;0 || slice&gt;=slices() ||
 * row&lt;0 || row&gt;=rows() || column&lt;0 || column&gt;=column()</tt> will
...
public abstract class ObjectMatrix3D extends AbstractMatrix3D {
     ...
     * value <tt>v</tt> such that <tt>v==a(size())</tt> where <tt>a(i) == aggr(
     * a(i-1), f(get(slice,row,column)) )</tt> and terminators are <tt>a(1) ==
     * f(get(0,0,0)), a(0)==null</tt>.
     * <p>
     * <b>Example:</b>
     * <pre>
     ...
     * <tt>v==a(size())</tt> where <tt>a(i) == aggr( a(i-1),
     * f(get(slice,row,column),other.get(slice,row,column)) )</tt> and
     * terminators are <tt>a(1) == f(get(0,0,0),other.get(0,0,0)),
     * a(0)==null</tt>.
     * <p>
     * <b>Example:</b>
     * <pre>
     ...
     *
     * @param aggr an aggregation function taking as first argument the current
     * aggregation and as second argument the transformed current cell values.
     * @param f a function transforming the current cell values.
     * @return the aggregated measure.
     * @throws IllegalArgumentException if <tt>slices() != other.slices() ||
     * rows() != other.rows() || columns() != other.columns()</tt>
     * @see cern.jet.math.Functions
     */
     ...
     * @throws IllegalArgumentException if <tt>values.length != slices() || for
     * any 0 &lt;= slice &lt; slices(): values[slice].length != rows()</tt>.
     * @throws IllegalArgumentException if <tt>for any 0 &lt;= column &lt;
     * columns(): values[slice][row].length != columns()</tt>.
     */
     ...
     * href="package-summary.html#FunctionObjects">package doc</a>.
     *
     * @param function a function object taking as argument the current cell's
     * value.
     * @return <tt>this</tt> (for convenience only).
     * @see cern.jet.math.Functions
     */
     ...
     * @param function a function object taking as first argument the current
     * cell's value of <tt>this</tt>, and as second argument the current cell's
     * value of <tt>y</tt>,
     * @return <tt>this</tt> (for convenience only).
     * @throws IllegalArgumentException if <tt>slices() != other.slices() ||
     * rows() != other.rows() || columns() != other.columns()</tt>
     ...
     * The returned matrix is not backed by this matrix, so changes in the
     * returned matrix are not reflected in this matrix, and vice-versa.
     *
    ...
    public boolean equals(Object otherObj, boolean testForEquality) { //delta
        ...
        if (!testForEquality) {
            ...
            for (int slice = slices; --slice >= 0; ) {
                for (int row = rows; --row >= 0; ) {
                    for (int column = columns; --column >= 0; ) {
                        if (!(getQuick(slice, row, column) == null ? other.getQuick(slice, row, column) == null : getQuick(slice, row, column).equals(other.getQuick(slice, row, column))))
                            return false;
                    }
                }
            }
     ...
     * @throws IndexOutOfBoundsException if <tt>slice&lt;0 || slice&gt;=slices()
     * || row&lt;0 || row&gt;=rows() || column&lt;0 ||
     * column&gt;=column()</tt>.
     */
     ...
     * like: <tt>for (slice = 0..slices-1) for (row = 0..rows-1) for (column =
     * 0..colums-1) do ... </tt>. However, subclasses are free to us any other
     * order, even an order that may change over time as cell values are
     * changed. (Of course, result lists indexes are guaranteed to correspond to
     * the same cell). For an example, see
     * {@link ObjectMatrix2D#getNonZeros(IntArrayList, IntArrayList,
     * ObjectArrayList)}.
     *
     * @param sliceList the list to be filled with slice indexes, can have any
     * size.
     * @param rowList the list to be filled with row indexes, can have any
     * size.
     * @param columnList the list to be filled with column indexes, can have any
     * size.
     * @param valueList the list to be filled with values, can have any size.
     */
    public void getNonZeros(IntArrayList sliceList, IntArrayList rowList, IntArrayList columnList, ObjectArrayList<Object> valueList) {
        sliceList.clear();
        rowList.clear();
        columnList.clear();
        valueList.clear();
     ...
     * Precondition (unchecked): <tt>slice&lt;0 || slice&gt;=slices() ||
     ...
     * @throws IndexOutOfBoundsException if <tt>row&lt;0 || row&gt;=rows() ||
     * slice&lt;0 || slice&gt;=slices() || column&lt;0 ||
     * column&gt;=column()</tt>.
     */
     ...
     * The returned matrix is backed by this matrix, so changes in the returned
     ...
     * (<tt>view().part(...)</tt>), then apply this method to the sub-range
     * view. To obtain 1-dimensional views, apply this method, then apply
     * another slice view (methods <tt>viewColumn</tt>, <tt>viewRow</tt>) on the
     ...
     * @throws IndexOutOfBoundsException if <tt>column < 0 || column >=
     * columns()</tt>.
     * @see #viewSlice(int)
     * @see #viewRow(int)
     */
     ...
     * @throws IllegalArgumentException if some of the parameters are equal or
     * not in range 0..2.
     */
    ...
    /**
     * Constructs and returns a new <i>sub-range view</i> that is a <tt>depth x
     * height x width</tt> sub matrix starting at <tt>[slice,row,column]</tt>;
     * Equivalent to <tt>view().part(slice,row,column,depth,height,width)</tt>;
     * Provided for convenience only. The returned view is backed by this
     ...
     * @return the new view.
     * @throws IndexOutOfBoundsException if <tt>slice<0 || depth<0 ||
     * slice+depth>slices() || row<0 || height<0 || row+height>rows() ||
     * column<0 || width<0 || column+width>columns()</tt>
     */
     ...
     * holding the indicated cells. There holds <tt>view.slices() ==
     * sliceIndexes.length, view.rows() == rowIndexes.length, view.columns() ==
     * columnIndexes.length</tt> and
     * <tt>view.get(k,i,j) ==
     * this.get(sliceIndexes[k],rowIndexes[i],columnIndexes[j])</tt>.
     ...
     * vice-versa.
     *
     * @param sliceIndexes The slices of the cells that shall be visible in the
     * new view. To indicate that <i>all</i> slices shall be visible, simply set
     * this parameter to <tt>null</tt>.
     * @param rowIndexes The rows of the cells that shall be visible in the new
     * view. To indicate that <i>all</i> rows shall be visible, simply set this
     * parameter to <tt>null</tt>.
     * @param columnIndexes The columns of the cells that shall be visible in
     * the new view. To indicate that <i>all</i> columns shall be visible,
     * simply set this parameter to <tt>null</tt>.
     * @return the new view.
     * @throws IndexOutOfBoundsException if <tt>!(0 <= sliceIndexes[i] <
     * slices())</tt> for any <tt>i=0..sliceIndexes.length()-1</tt>.
     * @throws IndexOutOfBoundsException if <tt>!(0 <= rowIndexes[i] <
     * rows())</tt> for any <tt>i=0..rowIndexes.length()-1</tt>.
     * @throws IndexOutOfBoundsException if <tt>!(0 <= columnIndexes[i] <
     * columns())</tt> for any <tt>i=0..columnIndexes.length()-1</tt>.
     */
     ...
     * @return a new 2-dimensional slice view.
     * @throws IndexOutOfBoundsException if <tt>slice < 0 || slice >=
     * slices()</tt>.
     * @see #viewRow(int)
     * @see #viewColumn(int)
     */
     ...
     * information, see
     * {@link cern.colt.matrix.objectalgo.Sorting#sort(ObjectMatrix3D, int,
     * int)}. For more advanced sorting functionality, see
     * {@link cern.colt.matrix.objectalgo.Sorting}.
     *
     * @return a new sorted vector (matrix) view.
     * @throws IndexOutOfBoundsException if <tt>row < 0 || row >= rows() ||
     * column < 0 || column >= columns()</tt>.
     */
     ...
     * <tt>this.get(k*sliceStride,i*rowStride,j*columnStride)</tt> for all <tt>k
     * = 0..slices()/sliceStride - 1, i = 0..rows()/rowStride - 1, j =
     * 0..columns()/columnStride - 1</tt>. The returned view is backed by this
     * matrix, so changes in the returned view are reflected in this matrix, and
     * vice-versa.
     *
     ...
     * <tt>[slices()-1,rows()-1,columns()-1]</tt> to <tt>[0,0,0]</tt>, as
     ...
     * return true;
     * </pre>
     * Note that an implementation may use more efficient techniques, but must
     * not use any other order.
     *
     * @param procedure a procedure object taking as argument the current cell's
     * value. Stops iteration if the procedure returns <tt>false</tt>, otherwise
     * continues.
     ...
     * not use any other order.
     *
     * @param procedure a procedure object taking as first argument the current
     * slice, as second argument the current row, and as third argument the
     * current column. Stops iteration if the procedure returns <tt>false</tt>,
     * otherwise continues.
     * @return <tt>false</tt> if the procedure stopped before all elements where
     * iterated over, <tt>true</tt> otherwise.
     */
