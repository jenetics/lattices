...
class Smp {
	...
	protected void run(
		...
		for (int i = 0; i < blocksA.length; i++) {
			...
			subTasks[i] = ForkJoinTask.adapt(() -> {
				final double result = function.apply(
					blocksA[k],
					blocksB != null ? blocksB[k] : null
				);
				if (results != null) {
					results[k] = result;
				}
	...
	*/
		...
		int noOfTasks = (int) Math.min(flops / threshold, ForkJoinPool.getCommonPoolParallelism()); // each thread should process at least 30000 flops
