...
public class Property extends cern.colt.PersistentObject {
     ...
     *
     */
    protected static String get(cern.colt.list.ObjectArrayList<String> list, int index) {
    ...
    public void checkSquare(DoubleMatrix2D A) {
        if (A.rows() != A.columns())
            throw new IllegalArgumentException("Matrix must be square: " + cern.colt.matrix.doublealgo.Formatter.shape(A));
    }
     ...
     * given value. The result is <tt>true</tt> if and only if <tt>A !=
     * null</tt> and
     * <tt>! (Math.abs(value - A[i]) > tolerance())</tt> holds for all
     * coordinates.
     *
     ...
     * absolute values of the off-diagonal elements in the corresponding
     * column.
     * <tt>returns true if for all i: abs(A[i,i]) &gt; Sum(abs(A[j,i])); j !=
     * i.</tt>
     ...
     * <tt>returns true if for all i: abs(A[i,i]) &gt; Sum(abs(A[i,j])); j !=
     * i.</tt>
     * Matrix may but need not be square.
     * <p>
     * Note: Ignores tolerance.
    ...
    /**
     * A square matrix <tt>A</tt> is <i>orthogonal</i> if <tt>A*transpose(A) =
     * I</tt>.
     *
     * @throws IllegalArgumentException if <tt>!isSquare(A)</tt>.
     */
     ...
     * -transpose(A)</tt>, that is <tt>A[i,j] == -A[j,i]</tt>.
     *
     * @throws IllegalArgumentException if <tt>!isSquare(A)</tt>.
     */
     ...
     *
     * @throws UnsupportedOperationException if <tt>this==DEFAULT || this==ZERO
     * || this==TWELVE</tt>.
     */
