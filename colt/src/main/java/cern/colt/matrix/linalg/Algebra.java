...
/**
 * Linear algebraic matrix operations operating on {@link DoubleMatrix2D};
...
public class Algebra extends cern.colt.PersistentObject {
    /**
     * A default Algebra object; has {@link Property#DEFAULT} attached for
     * tolerance. Allows ommiting to construct an Algebra object time and
     * again.
     * <p>
    ...
    /**
     * A default Algebra object; has {@link Property#ZERO} attached for
     * tolerance. Allows ommiting to construct an Algebra object time and
     * again.
     * <p>
     * Note that this Algebra object is immutable. Any attempt to assign a new
     ...
     * @throws IllegalArgumentException if <tt>A.rows() != x.size() ||
     * A.columns() != y.size()</tt>.
     */
...
//	return max;
     ...
     * <tt>indexes.length==A.size() && indexes[i] >= 0 && indexes[i] <
     * A.size()</tt>;
     * @param work the working storage, must satisfy <tt>work.length >=
     * A.size()</tt>; set <tt>work==null</tt> if you don't care about
     * performance.
     * @return the modified <tt>A</tt> (for convenience only).
     * @throws IndexOutOfBoundsException if <tt>indexes.length !=
     * A.size()</tt>.
     */
	...
	*/
     ...
     * in this matrix, and vice-versa. Use idioms like <tt>result =
     * permute(...).copy()</tt> to generate an independent sub matrix.
     *
     ...
     * @param indexes the permutation indexes, must satisfy
     * <tt>indexes.length==A.columns() && indexes[i] >= 0 && indexes[i] <
     * A.columns()</tt>;
     * @param work the working storage, must satisfy <tt>work.length >=
     * A.columns()</tt>; set <tt>work==null</tt> if you don't care about
     * performance.
     * @return the modified <tt>A</tt> (for convenience only).
     * @throws IndexOutOfBoundsException if <tt>indexes.length !=
     * A.columns()</tt>.
     ...
     * @param A the matrix to permute.
     * @param indexes the permutation indexes, must satisfy
     * <tt>indexes.length==A.rows() && indexes[i] >= 0 && indexes[i] <
     * A.rows()</tt>;
     * @param work the working storage, must satisfy <tt>work.length >=
     * A.rows()</tt>; set <tt>work==null</tt> if you don't care about
     * performance.
     * @return the modified <tt>A</tt> (for convenience only).
     * @throws IndexOutOfBoundsException if <tt>indexes.length !=
     * A.rows()</tt>.
	...
	*/
    ...
    /**
     * Linear algebraic matrix power; <tt>B = A<sup>k</sup> <==> B =
     * A*A*...*A</tt>.
     * <ul>
	...
	*/
    ...
    /**
     * Returns the property object attached to this Algebra, defining
     * tolerance.
     *
     ...
     * @throws UnsupportedOperationException if <tt>this==DEFAULT &&
     * property!=this.property()</tt> - The DEFAULT Algebra object is
     * immutable.
     * @throws UnsupportedOperationException if <tt>this==ZERO &&
     * property!=this.property()</tt> - The ZERO Algebra object is immutable.
     * @see #property
     */
     ...
     * A[rowIndexes(:),columnFrom..columnTo]</tt>;
     ...
     * @throws IndexOutOfBoundsException if <tt>columnFrom<0 ||
     * columnTo-columnFrom+1<0 || columnTo+1>matrix.columns() || for any
     * row=rowIndexes[i]: row < 0 || row >= matrix.rows()</tt>.
     */
     ...
     * <tt>sub[0..rowTo-rowFrom,0..columnIndexes.length-1] =
     * A[rowFrom..rowTo,columnIndexes(:)]</tt>;
     * The returned matrix is <i>not backed</i> by this matrix, so changes in
     * the returned matrix are <i>not reflected</i> in this matrix, and
     * vice-versa.
     *
     ...
     * @param columnIndexes the indexes of the columns to copy. May be
     * unsorted.
     ...
     * reflected in this matrix, and vice-versa. Use idioms like <tt>result =
     * subMatrix(...).copy()</tt> to generate an independent sub matrix.
     *
     * @param A the source matrix.
     ...
     * @throws IndexOutOfBoundsException if <tt>fromColumn<0 ||
     * toColumn-fromColumn+1<0 || toColumn>=A.columns() || fromRow<0 ||
     * toRow-fromRow+1<0 || toRow>=A.rows()</tt>
     */
    ...
    public String toString(DoubleMatrix2D matrix) {
        final cern.colt.list.ObjectArrayList<String> names = new cern.colt.list.ObjectArrayList<>();
        final cern.colt.list.ObjectArrayList<Object> values = new cern.colt.list.ObjectArrayList<>();
...
*/
    ...
    /**
     * Outer product of two vectors; Returns a matrix with <tt>A[i,j] = x[i] *
     * y[j]</tt>.
     *
     * @param x the first source vector.
     * @param y the second source vector.
